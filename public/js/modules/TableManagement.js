...
//!BU MODUL OLAMAZ BIR ARA DUZENLE
...
export default class TableManagement {
  ...
  async getTables() {
    ...
    const response = await requestHandler.post(
      baseUrl + "terminal-service/get-tables"
    );
      ...
      if (element.table_status === "close") {
        ...
        display += `
            ...
            >
                ...
                <div class="table-id">${element.table_area} ${
          element.table_number
        }</div>

                <div id="table-status-${
                  element.id
                }" class="table-status"><span>${this.formatter.moneyFormatter(
          element.table_amount,
          ...
          "TRY"
                ...
                <div class="table-meta-data">
                    ...
                    <span id="table-waiter-${element.id}">${
          element.first_waiter
        }</span>

                    <span id="table-amount-${
                      element.id
                    }">${hour}:${minutes}</span>
  ...
  tableClickEventAdder() {
    ...
    buttons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        const selectedTableTotalAmount = button.getAttribute(
  ...
  async getTablesDataUpdate() {
    ...
    tablesData.forEach((element) => {
      if (element.table_status === "close") {
        ...
        const tableStatus = document.getElementById(
          `table-status-${element.id}`
        );

        const waiterName = document.getElementById(
          `table-waiter-${element.id}`
        );

        const tableAmount = document.getElementById(
          `table-amount-${element.id}`
        );
        ...
        tableStatus.innerText = `${this.formatter.moneyFormatter(
          element.table_amount,
